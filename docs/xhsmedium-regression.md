...
Compose cleanup 不等于完整的磁盘 cleanup。业务证据归档后，RRM-D2 helper 会删除本轮精确 `xhsmedium-test-scheduled-<runId>-{backend,frontend,runner}`运行镜像，拒绝越界名称并输出零残留证据；该行为已通过固定 SHA 的 paper-server 成功、失败、超时和人工中断全量回归。允许的 SHA 依赖缓存窗口与 DIND BuildKit cache 由 RRM-D3.1 Maintenance Job治理。RRM-D4在归档后按固定 Artifact 路径收敛失败 trace，非 pin构建最多保留最近2份且最长7天，并将完整 Artifact保留限制为最近5个构建；统一规则见[`runtime-residue-management.md`](runtime-residue-management.md)。
