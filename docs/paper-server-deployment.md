...
磁盘治理采用分级水位：30 GiB及以上正常，25–30 GiB预警并只允许一个重型任务，20–25 GiB自动拒绝新的 Regression、Release 和 Deploy，低于 20 GiB进入紧急处置。Pipeline 使用 Jenkins `DiskSpaceMonitor`遥测，不挂载宿主根目录；专用 Maintenance保留为低磁盘精确恢复通道。运行镜像、DIND cache、Jenkins Artifact 和 trace 的保留矩阵及清理证据见[`runtime-residue-management.md`](runtime-residue-management.md)。禁止以宿主全局 prune 代替按 runId、SHA、Build 或专用 DIND 的精确清理。
