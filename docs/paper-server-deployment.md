...
如果 paper-server 到业务锁文件记录的包地址持续超时，可以从受控构建主机传入使用相同完整 SHA 和锁定基础镜像生成的三个 `xhsmedium-deps-<short-sha>-*`镜像。预加载器通过 `--cache-from`复用这些精确层，仍会重新解析 Dockerfile、固定 SHA 上下文和标签，并在导入 DIND 后验证完整 SHA 与角色；禁止修改业务锁文件、替换包地址或把失败构建报告为通过。
