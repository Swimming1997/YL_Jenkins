...
try {
    ...
    Assert-True ($maintenanceDsl -match 'Refusing unsafe Maintenance temporary cleanup path' -and $maintenanceDsl -match 'dind-\(regression\|release\|deploy-dev\|deploy-test\)@tmp' -and $maintenanceDsl -match 'dir\(maintenanceTmp\) \{ deleteDir\(\) \}') 'Maintenance removes only its exact sibling Jenkins temporary Workspace after every result.'
