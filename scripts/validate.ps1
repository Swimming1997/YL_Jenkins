...
try {
    ...
    Assert-True ($xhsmediumCi -match "(?s)stage\('Backend'\).*?NODE_OPTIONS = '--max-old-space-size=768'" -and ([regex]::Matches($xhsmediumCi, "NODE_OPTIONS = '--max-old-space-size=512'")).Count -eq 3) 'XHSMedium CI applies bounded per-module Node heap limits.'
