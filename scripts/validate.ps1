...
try {
    ...
    Assert-True ($paperProfile -match 'rollback_start=true' -and $paperProfile -match 'PROFILE_LIFECYCLE_CLEANUP' -and $paperProfile -match 'PROFILE_LIFECYCLE_EVIDENCE' -and $paperProfile -notmatch '(?i)down\s+--volumes|system\s+prune|volume\s+prune') 'paper-server lifecycle rolls back failed fresh starts, emits evidence, and contains no destructive global cleanup.'
