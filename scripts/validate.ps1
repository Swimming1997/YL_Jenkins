...
try {
    ...
    Assert-True (([regex]::Matches($paperPreloadScript, 'type=local,src=\$repo_root/\.secrets/p4-preload-build-cache-\$short_sha/(?:backend|frontend|runner)')).Count -eq 3) 'paper-server preloader can reuse explicit local BuildKit caches without Registry access or changing fixed-SHA inputs.'
