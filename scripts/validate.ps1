...
try {
    ...
    Assert-True ($dindMaintenance -match 'builder prune --all --force --max-used-space' -and $dindMaintenance -match '4294967296' -and $dindMaintenance -match 'DIND_MAINTENANCE_EVIDENCE' -and $dindMaintenance -notmatch '(?i)(?:system|volume)\s+prune') 'DIND helper bounds all unused cache records in only its dedicated BuildKit daemon and emits structured evidence without global prune.'
