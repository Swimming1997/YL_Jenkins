...
try {
    ...
    Assert-True ($maintenanceDsl -match 'Refusing unsafe Maintenance temporary cleanup path' -and ([regex]::Matches($maintenanceDsl, '/home/jenkins/agent/workspace/Platform/Maintenance/dind-(?:regression|release|deploy-dev|deploy-test)@tmp')).Count -eq 4 -and $maintenanceDsl -match 'allowedMaintenanceTmp\.contains\(maintenanceTmp\)' -and $maintenanceDsl -match 'dir\(maintenanceTmp\) \{ deleteDir\(\) \}') 'Maintenance removes only one of four exact sibling Jenkins temporary Workspaces after every result.'
