...
try {
    ...
    foreach ($configFile in @('jcasc\jenkins.yaml', 'jcasc\security.yaml', 'jcasc\authorization.yaml', 'jcasc\jobs.yaml', 'jcasc\agents.yaml', 'jcasc\credentials.yaml', 'jobs\folders.groovy', 'jobs\seed.groovy', 'jobs\projects\xhsmedium-ci.groovy', 'jobs\projects\xhsmedium-regression.groovy', 'jobs\projects\xhsmedium-release.groovy', 'jobs\projects\xhsmedium-deploy.groovy', 'jobs\projects\runtime-maintenance.groovy', 'jobs\resources\xhsmedium-deploy-compose.yaml', 'shared-library\vars\validateGitRef.groovy', 'shared-library\vars\nodeModuleCi.groovy', 'shared-library\vars\recordBuildMetadata.groovy', 'shared-library\vars\scmChangeDecision.groovy', 'shared-library\vars\paperServerResourceGate.groovy', 'shared-library\vars\retainRegressionTraces.groovy', 'shared-library\src\org\swimming1997\jenkins\TraceRetentionPolicy.groovy', 'shared-library\resources\xhsmedium\npm-ci-network-retry.sh', 'shared-library\resources\xhsmedium\docker-offline-wrapper.sh', 'shared-library\resources\xhsmedium\mysql-entrypoint-compat.yaml', 'shared-library\resources\xhsmedium\mysql-init-wrapper.sh', 'shared-library\resources\xhsmedium\scheduled-slot-shim.cjs', 'shared-library\resources\xhsmedium\runner-volume-entrypoint.sh', 'shared-library\resources\xhsmedium\docker-project-cleanup.sh', 'shared-library\resources\xhsmedium\docker-run-image-cleanup.sh', 'shared-library\resources\xhsmedium\docker-dind-maintenance.sh', 'shared-library\resources\xhsmedium\paper-server-resource-gate.py', 'scripts\preload-xhsmedium-regression.ps1', 'scripts\preload-xhsmedium-regression.sh', 'scripts\paper-server-profile.sh', 'scripts\test-paper-server-profile.ps1', 'scripts\test-xhsmedium-regression.ps1', 'scripts\test-xhsmedium-regression-paper-server.sh', 'scripts\test-xhsmedium-regression-resilience.ps1', 'scripts\test-docker-run-image-cleanup.ps1', 'scripts\test-dind-maintenance.ps1', 'scripts\test-resource-gate.ps1', 'scripts\test-rrm-d4-runtime.ps1', 'scripts\test-xhsmedium-watcher.ps1', 'scripts\test-xhsmedium-release.ps1', 'scripts\test-xhsmedium-deploy.ps1', 'scripts\test-xhsmedium-deploy-cross-version.ps1', 'scripts\test-npm-ci-network-retry.ps1', 'scripts\test-backup-restore.ps1', 'scripts\test-disk-pressure.ps1', 'scripts\test-retention.ps1', 'scripts\test-hardening.ps1', 'scripts\bootstrap-paper-server.sh', 'compose.paper-server.yaml', 'docs\platform-hardening.md', 'docs\runtime-residue-management.md', 'docs\operations.md', 'docs\incident-response.md', 'docs\onboarding-project.md', 'docs\xhsmedium-release.md', 'docs\xhsmedium-deployment.md', 'docs\registry-cloud-deployment.md', 'docs\paper-server-deployment.md', 'agents\build\Dockerfile', 'agents\regression\Dockerfile', 'agents\release\Dockerfile', 'agents\release\entrypoint.sh', 'agents\deploy\Dockerfile', 'agents\deploy\entrypoint.sh')) {
    ...
    Assert-True ($jenkinsConfig -match 'key:\s*"PAPER_SERVER_RESOURCE_MODE"' -and $jenkinsConfig -match 'value:\s*"\$\{PAPER_SERVER_RESOURCE_MODE\}"') 'paper-server resource mode is propagated to Pipeline environments through JCasC.'

    $authorizationConfig = Get-Content -Raw -LiteralPath 'jcasc\authorization.yaml'
    Assert-True ($authorizationConfig -notmatch 'Computer/(?:ExtendedRead|Configure|Connect|Create|Delete|Disconnect)') 'Resource gate introduces no Computer permission for audit or authenticated users.'
    ...
    Assert-True ($libraryConfig -match 'job-dsl/projects/runtime-maintenance\.groovy') 'Dedicated DIND Maintenance Job DSL is loaded by JCasC.'

    $maintenanceDsl = Get-Content -Raw -LiteralPath 'jobs\projects\runtime-maintenance.groovy'
    Assert-True ($maintenanceDsl -match "regression: \[label: 'xhsmedium-regression'" -and $maintenanceDsl -match "release: \[label: 'xhsmedium-release'" -and $maintenanceDsl -match "'deploy-dev': \[label: 'xhsmedium-deploy-dev'" -and $maintenanceDsl -match "'deploy-test': \[label: 'xhsmedium-deploy-test'") 'Four Maintenance Jobs are pinned to their dedicated DIND Agents.'
    Assert-True ($maintenanceDsl -match 'pipelineJob\("Platform/Maintenance/dind-\$\{targetName\}"\)' -and $maintenanceDsl -match 'disableConcurrentBuilds\(abortPrevious: false\)' -and $maintenanceDsl -match 'APPLY_DEDICATED_DIND_MAINTENANCE') 'Maintenance Jobs are non-concurrent and require exact APPLY confirmation.'
    Assert-True ($maintenanceDsl -match '\$\{CURRENT_VALIDATED_SHA:-\}' -and $maintenanceDsl -match '\$\{PREVIOUS_VALIDATED_SHA:-\}' -and $maintenanceDsl -match '\$\{CONFIRMATION:-\}') 'Maintenance helper invocation tolerates optional empty parameters under shell nounset mode.'
    Assert-True ($maintenanceDsl -match "credentialsId: 'jenkins-audit-api'" -and $maintenanceDsl -match 'JENKINS_QUEUE_NOT_EMPTY' -and $maintenanceDsl -match 'OTHER_JENKINS_BUILD_RUNNING' -and $maintenanceDsl -match 'CURRENT_SHA_NOT_LATEST_VALIDATED' -and $maintenanceDsl -match 'PREVIOUS_SHA_NOT_VALIDATED_WINDOW') 'Maintenance verifies the empty queue, idle executors, and Regression validated-SHA window through the read-only API credential.'
    Assert-True (-not (($maintenanceDsl -split "`r?`n") | Where-Object { $_ -match "python3 -c.*(?<!\\)\\'" })) 'Nested Maintenance Pipeline preserves escaped Python string quotes through Job DSL generation.'
    Assert-True ($maintenanceDsl -match "libraryResource\('xhsmedium/docker-dind-maintenance\.sh'\)" -and $maintenanceDsl -notmatch '/var/run/docker\.sock|(?i)docker\s+system\s+prune|docker\s+volume\s+prune') 'Maintenance installs the reviewed helper and never accesses the host Docker Socket or global prune commands.'
    Assert-True ($maintenanceDsl -match 'maintenanceTmp = .*\.toString\(\)' -and $maintenanceDsl -match 'Refusing unsafe Maintenance temporary cleanup path' -and ([regex]::Matches($maintenanceDsl, '/home/jenkins/agent/workspace/Platform/Maintenance/dind-(?:regression|release|deploy-dev|deploy-test)@tmp')).Count -eq 4 -and $maintenanceDsl -match 'allowedMaintenanceTmp\.contains\(maintenanceTmp\)' -and $maintenanceDsl -match 'dir\(maintenanceTmp\) \{ deleteDir\(\) \}') 'Maintenance normalizes and removes only one of four exact sibling Jenkins temporary Workspaces after every result.'

    $dindMaintenance = Get-Content -Raw -LiteralPath 'shared-library\resources\xhsmedium\docker-dind-maintenance.sh'
    Assert-True ($dindMaintenance -match 'regression\|release\|deploy-dev\|deploy-test' -and $dindMaintenance -match 'APPLY_DEDICATED_DIND_MAINTENANCE' -and $dindMaintenance -match 'Refusing maintenance while target DIND has running containers') 'DIND helper validates its exact target, confirmation, and empty runtime before mutation.'
    Assert-True ($dindMaintenance -match '\^xhsmedium-deps-\(\[0-9a-f\]\{8\}\)-\(backend\|frontend\|runner\):latest\$' -and $dindMaintenance -match 'Maintenance removed a protected image' -and $dindMaintenance -match 'CURRENT_VALIDATED_SHA|current_sha') 'DIND helper only selects exact expired Regression cache roles and verifies protected images remain.'
    Assert-True ($dindMaintenance -match 'builder prune --all --force --max-used-space' -and $dindMaintenance -match 'builder prune --all --force >/dev/null' -and $dindMaintenance -match 'cache_fallback=' -and $dindMaintenance -match '4294967296' -and $dindMaintenance -match 'DIND_MAINTENANCE_EVIDENCE' -and $dindMaintenance -notmatch '(?i)(?:system|volume)\s+prune') 'DIND helper bounds all unused cache records in only its dedicated BuildKit daemon, uses a recorded fallback, and emits evidence without global prune.'
    ...
    Assert-True ($resourceGateStep -match "new File\('/var/jenkins_home'\)" -and $resourceGateStep -match 'jenkinsHome\.usableSpace' -and $resourceGateStep -match "credentialsId: 'jenkins-audit-api'" -and $resourceGateStep -match 'computer/api/json\?tree=computer' -and $resourceGateStep -notmatch 'computer/\(built-in\)|/var/run/docker\.sock|(?i)docker\s+(?:system|volume)\s+prune') 'Trusted resource gate reads Controller Jenkins Home space, uses audit only for executor state, and has no host Docker access.'
    ...
    Assert-True ($tracePolicy -match 'MAX_FAILURE_TRACES = 2' -and $tracePolicy -match '7L \* 24L' -and $tracePolicy -match 'playwright\[\^/\]\*-report/data/\[\^/\]\+\\\\\.zip' -and $tracePolicy -match 'build\.keepLog' -and $tracePolicy -notmatch 'eachFileRecurse|\.sort\s*\{|\.collect\s*\{|\.each\s*\{|\.findAll\s*\{|\.collectMany\s*\{') 'Trace retention keeps at most two seven-day failure trace files, restricts exact paths, exempts pinned builds, and avoids non-CPS collection closures.'
    ...
    $retentionDsl = @(
        Get-Content -Raw -LiteralPath 'jobs\projects\xhsmedium-ci.groovy'
        Get-Content -Raw -LiteralPath 'jobs\projects\xhsmedium-regression.groovy'
        Get-Content -Raw -LiteralPath 'jobs\projects\xhsmedium-release.groovy'
        Get-Content -Raw -LiteralPath 'jobs\projects\xhsmedium-deploy.groovy'
        Get-Content -Raw -LiteralPath 'jobs\projects\runtime-maintenance.groovy'
    ) -join "`n"
    Assert-True (-not ($retentionDsl -match "numToKeepStr: '30'|numToKeep\(30\)")) 'Managed CI, Regression, Release, Deploy, and Maintenance Jobs no longer retain 30 build records.'
    Assert-True (([regex]::Matches($retentionDsl, "artifactNumToKeepStr: '5'")).Count -eq 7 -and ([regex]::Matches($retentionDsl, 'artifactNumToKeep\(5\)')).Count -eq 7) 'All seven managed Pipeline templates retain 20 build records and five complete Artifact sets.'

    $regressionDsl = Get-Content -Raw -LiteralPath 'jobs\projects\xhsmedium-regression.groovy'
    $releaseDsl = Get-Content -Raw -LiteralPath 'jobs\projects\xhsmedium-release.groovy'
    $deployDsl = Get-Content -Raw -LiteralPath 'jobs\projects\xhsmedium-deploy.groovy'
    Assert-True (([regex]::Matches($regressionDsl, 'paperServerResourceGate\(\)')).Count -eq 1 -and ([regex]::Matches($releaseDsl, 'paperServerResourceGate\(\)')).Count -eq 2 -and ([regex]::Matches($deployDsl, 'paperServerResourceGate\(\)')).Count -eq 1) 'Regression, both Release Pipelines, and both generated Deploy Jobs enter the paper-server resource gate before workload stages.'
    Assert-True ($maintenanceDsl -notmatch 'paperServerResourceGate\(') 'Dedicated Maintenance remains available as the bounded low-disk recovery path.'
    Assert-True ($regressionDsl.IndexOf('retainRegressionTraces(result: currentBuild.currentResult)') -gt $regressionDsl.IndexOf("archiveArtifacts artifacts: 'scheduled-regression.log")) 'Regression applies trace retention only after Artifact archival.'
    $rrmRuntimeTest = Get-Content -Raw -LiteralPath 'scripts\test-rrm-d4-runtime.ps1'
    Assert-True ($rrmRuntimeTest -notmatch '(?i)down\s+--volumes|down\s+-v' -and $rrmRuntimeTest -match 'docker volume rm \$volumeName' -and $rrmRuntimeTest -match 'docker network rm \$networkName') 'RRM-D4 runtime test deletes only its unique Controller, volume, and network.'
    ...
    Assert-True ($xhsmediumDeploy -match 'failed_approval_build="\$APPROVAL_BUILD"' -and $xhsmediumDeploy -match 'FAILED_APPROVAL_BUILD="\$failed_approval_build" python3' -and $xhsmediumDeploy -match 'failedApprovalBuild.*FAILED_APPROVAL_BUILD' -and $xhsmediumDeploy -match 'P6_DEPLOY_ROLLED_BACK.*approval=\$failed_approval_build') 'P6 rollback preserves the failed Approval before loading the previous deployment environment and uses it in JSON and console evidence.'
    $crossVersionDeployTest = Get-Content -Raw -LiteralPath 'scripts\test-xhsmedium-deploy-cross-version.ps1'
    Assert-True ($crossVersionDeployTest -match '\[Parameter\(Mandatory\)\].*\$BaselineApprovedReleaseBuild' -and $crossVersionDeployTest -match '\[Parameter\(Mandatory\)\].*\$CandidateApprovedReleaseBuild' -and $crossVersionDeployTest -match "ValidateSet\('all', 'dev', 'test'\)" -and $crossVersionDeployTest -match 'complete environment triple' -and $crossVersionDeployTest -match 'preserve baseline, rollback, candidate execution order') 'P6 cross-version validation requires two Approvals, supports one serial environment, and rejects partial or misordered historical evidence triples.'
    Assert-True ($crossVersionDeployTest -match 'Baseline A and candidate B use different Git SHAs' -and $crossVersionDeployTest -match 'different backend digests' -and $crossVersionDeployTest -match 'different frontend digests') 'P6 cross-version validation requires distinct SHAs and both image digests.'
    Assert-True ($crossVersionDeployTest -match '(?s)ApprovalBuild \$BaselineApprovedReleaseBuild -SimulateFailure \$false.*?ApprovalBuild \$CandidateApprovedReleaseBuild -SimulateFailure \$true.*?ApprovalBuild \$CandidateApprovedReleaseBuild -SimulateFailure \$false') 'P6 cross-version validation orders baseline A, failed candidate B, and successful candidate B deployment.'
    Assert-True ($crossVersionDeployTest -match 'failedApprovalBuild.*CandidateApprovedReleaseBuild' -and $crossVersionDeployTest -match 'restoredApprovalBuild.*BaselineApprovedReleaseBuild' -and $crossVersionDeployTest -match 'rollback evidence restores both A digests' -and $crossVersionDeployTest -match 'P6_CROSS_VERSION_ROLLBACK_EVIDENCE') 'P6 cross-version validation directly proves failed B, restored A digests, and final structured evidence.'
    ...
    $runImageCleanup = Get-Content -Raw -LiteralPath 'shared-library\resources\xhsmedium\docker-run-image-cleanup.sh'
    Assert-True ($runImageCleanup -match '\^xhsmedium-test-scheduled-\[0-9\]' -and $runImageCleanup -match '\$project-backend:latest' -and $runImageCleanup -match '\$project-frontend:latest' -and $runImageCleanup -match '\$project-runner:latest' -and $runImageCleanup -match 'Refusing unexpected run image') 'Run image cleanup accepts only exact scheduled projects and three expected role tags.'
    Assert-True ($runImageCleanup -match 'image rm "\$image"' -and $runImageCleanup -match 'RESIDUE_CLEANUP_EVIDENCE' -and $runImageCleanup -match 'reclaimed_bytes=' -and $runImageCleanup -match 'disk_available_bytes=' -and $runImageCleanup -notmatch '(?i)(?:system|builder|volume)\s+prune') 'Run image cleanup removes exact tags, records disk evidence, and performs no prune operation.'
    $archiveStepIndex = $xhsmediumRegression.IndexOf("archiveArtifacts artifacts: 'scheduled-regression.log")
    $runImageCleanupStepIndex = $xhsmediumRegression.IndexOf('"$XHSMEDIUM_RUN_IMAGE_CLEANUP_PATH" "$XHSMEDIUM_DOCKER_PROJECT"')
    Assert-True ($xhsmediumRegression -match "libraryResource\('xhsmedium/docker-run-image-cleanup\.sh'\)" -and $archiveStepIndex -ge 0 -and $runImageCleanupStepIndex -gt $archiveStepIndex) 'Regression installs the run image helper and invokes it only after Artifact archival.'
    Assert-True ($xhsmediumRegression -match '(?s)archiveArtifacts.*?finally.*?XHSMEDIUM_RUN_IMAGE_CLEANUP_PATH.*?finally.*?cleanupWorkspace') 'Archive, run image cleanup, and Workspace cleanup remain ordered through nested finally blocks.'
    ...
    Assert-True ($paperPreloadScript -match '--import-existing' -and $paperPreloadScript -match 'verify_host_cache' -and $paperPreloadScript -match 'existing host cache verified') 'paper-server preloader can import prebuilt fixed-SHA caches only after host SHA and role verification.'
    Assert-True ($paperPreloadScript -match 'verify_host_cache_ready' -and $paperPreloadScript -match 'verify_dind_cache_ready' -and $paperPreloadScript -match 'npm ci --offline' -and $paperPreloadScript -match 'docker run --rm --network none') 'paper-server preloader fails closed unless host and DIND dependency caches pass an isolated offline-readiness probe.'
    ...
    Assert-True (([regex]::Matches($paperPreloadScript, "'docker\.m\.daocloud\.io/library/node:20-bookworm-slim'")).Count -ge 2 -and ([regex]::Matches($preloadScript, "'docker\.m\.daocloud\.io/library/node:20-bookworm-slim'")).Count -ge 2) 'Both regression preloaders transfer and verify the pinned frontend runtime Node image in DIND.'
    ...
    $paperProfile = Get-Content -Raw -LiteralPath 'scripts\paper-server-profile.sh'
    Assert-True ($paperProfile -match 'repo_root" != /opt/jenkins-platform' -and ([regex]::Matches($paperProfile, 'regression\|release\|deploy-dev\|deploy-test')).Count -ge 1 -and $paperProfile -match 'flock -n 9' -and $paperProfile -match 'reason=lifecycle_lock' -and $paperProfile -match 'other_heavy_profile' -and $paperProfile -match 'partial_profile_state') 'paper-server lifecycle accepts only its fixed root and four exact heavy profiles, with lock, serial, and partial-state gates.'
    Assert-True ($paperProfile -match '\.secrets/jenkins_admin_password' -and $paperProfile -match '\.secrets/jenkins_audit_password' -and $paperProfile -match 'chmod 0600' -and $paperProfile -match 'queue/api/json\?tree=items\[id\]' -and $paperProfile -match 'computer/api/json\?tree=computer\[executors' -and $paperProfile -match 'launchSlaveAgent') 'paper-server lifecycle scopes temporary credentials to audit activity checks and administrator node reconnect.'
    Assert-True ($paperProfile -match 'rollback_start=true' -and $paperProfile -match 'ensure_temp_root\s*\r?\n\s*case "\$action"' -and $paperProfile -match 'PROFILE_LIFECYCLE_CLEANUP' -and $paperProfile -match 'PROFILE_LIFECYCLE_EVIDENCE' -and $paperProfile -notmatch '(?i)down\s+--volumes|system\s+prune|volume\s+prune') 'paper-server lifecycle initializes cleanup state in the parent shell, rolls back failed fresh starts, emits evidence, and contains no destructive global cleanup.'
    $paperProfileTest = Get-Content -Raw -LiteralPath 'scripts\test-paper-server-profile.ps1'
    Assert-True ($paperProfileTest -match "'--network', 'none'" -and $paperProfileTest -match "'--read-only'" -and $paperProfileTest -match "'--cap-drop', 'ALL'" -and $paperProfileTest -match 'PROFILE_LIFECYCLE_TEST_EVIDENCE') 'paper-server lifecycle fault tests run in a networkless read-only Docker container and emit evidence.'
